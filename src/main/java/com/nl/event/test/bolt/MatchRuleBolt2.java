...
public class MatchRuleBolt2 extends BaseBasicBolt {
	private static final long serialVersionUID = 1L;
	private static final Logger LOG = LoggerFactory.getLogger(MatchRuleBolt2.class);
	...
	*//**
	 ...
	 * @param tablename 表名
	 *//*
	public MatchRuleBolt2(final DBCfg dbCfg, final RedisClusterCfg redisCfg,final String tablename) {
		LOG.info("instancing...");
		this.dbCfg = dbCfg;
	...
	public void execute(Tuple tuple, BasicOutputCollector collector) {
		...
		for(final RuleInstance ruleInstance:ruleInstanceList){			
			...
			final String eventValue = this.redisCluster.hexists(this.tablename + user.getMsisdn(), eventId) ? "true" : "false";
			saleActiveExpression=saleActiveExpression.replaceAll(ruleInstanceId, eventValue);		
		}
		...
		if (expression.evaluate().getBooleanValue()) {
			...
			if ("1".equals(saleActive.getIsCapture())) {
				...
				activeOpp.setMsisdn(user.getMsisdn());
				...
				collector.emit(new Values(activeOpp));//collector.emit(streams.get(Math.abs(user.getMsisdn().hashCode() % this.STREAMSIZE)),new Values(activeOpp));	
			}
		}
