...
public class OrRotateStrategy
    ...
    public boolean isRotationNeeded(final String data, final File file) {
        ...
        if (null != m_strategies) {
            ...
            for (int i = 0; i < length; i++) {
                if (m_strategies[i].isRotationNeeded(data, file)) {
                    m_usedRotation = i;
                    return true;
                }
            }
    ...
    public boolean isRotationNeeded(final long size, final File file) {
        m_usedRotation = -1;

        if (null != m_strategies) {
            final int length = m_strategies.length;
            for (int i = 0; i < length; i++) {
                if (m_strategies[i].isRotationNeeded(size, file)) {
                    m_usedRotation = i;
                    return true;
                }
            }
