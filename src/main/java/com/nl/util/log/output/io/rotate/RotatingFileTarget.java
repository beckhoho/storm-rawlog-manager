 ...
 * the LICENSE file.
 */
package com.nl.util.log.output.io.rotate;

import com.nl.util.log.format.Formatter;
import com.nl.util.log.output.io.FileTarget;
...
public class RotatingFileTarget extends FileTarget {
    ...
    private void getInitialFile() throws IOException {
        ...
        boolean rotate;
        if (m_fileStrategy.currentFile() != null && m_fileStrategy.currentFile().exists()) {
            rotate = m_rotateStrategy.isRotationNeeded(m_fileStrategy.currentFile().length(), m_fileStrategy.currentFile());
        } else {
            rotate = m_rotateStrategy.isRotationNeeded("", m_fileStrategy.currentFile());
        }
